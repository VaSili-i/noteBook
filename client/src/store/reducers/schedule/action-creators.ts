...
import {
    EnumScheduleAction,
    ErrorScheduleInterface,
    IDefaultSchedule,
    LoadingScheduleInterface,
    SucssesScheduleInterface
} from './types'


export const ScheduleCreators = {
    SetLoading: (loading: boolean): LoadingScheduleInterface => ({
        type: EnumScheduleAction.SET_IS_LOADING_SCHEDULE,
        payload: loading
    }),
    SetSucsses: (schedule: IDefaultSchedule[]): SucssesScheduleInterface => ({
        type: EnumScheduleAction.SUCSSES_SCHEDULE,
        payload: schedule
    }),
    SetError: (error: string): ErrorScheduleInterface => ({
