                ...
                {list.map((willDo, ind) =>
                    <div>
                        <Input value={willDo} ref={ind === 0 ? ref : null} key={ind}
                            onChange={(e) => { setList(list.map((item, index) => index === ind ? e.target.value : item)) }} />
                        <Button
                            onClick={() => { setList(list.filter((item, index) => index !== ind)) }}>
                            Delete</Button>
                    </div>
                )}
                <Button onClick={() => { setList([...list, '']) }}>add pole</Button>

            </div>
            {hides ?
                <div ref={isRef} onKeyDown={(e) => e.key == "Enter" ? addTypeSchedule() : null} onClick={(e) => { e.stopPropagation() }} className="createType">
                    <Input value={type} onChange={(e) => setType(e.target.value)} />
                    <Button onClick={() => { addTypeSchedule() }}>next</Button>
                </div>
                : null
            }
            <div onClick={(e) => { e.stopPropagation() }} className="leftMenuSchedule">
                {schedule.map((schedu: any, index: number) =>
                    <div onClick={() => { changeList(index) }}
